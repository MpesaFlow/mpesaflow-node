...
async function main() {
  const params: Mpesaflow.PaymentPaybillParams = {
    accountReference: 'accountReference',
    amount: 0,
    phoneNumber: 'phoneNumber',
    transactionDesc: 'transactionDesc',
  };
  const response: Mpesaflow.PaymentPaybillResponse = await client.payments.paybill(params);
}
...
await client.payments.paybill({ accountReference: 'accountReference', amount: 0, phoneNumber: 'phoneNumber', transactionDesc: 'transactionDesc' }, {
  maxRetries: 5,
});
```
