# Mpesaflow Node API Library

[![NPM version](https://img.shields.io/npm/v/mpesaflow.svg)](https://npmjs.org/package/mpesaflow) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/mpesaflow)
...
```sh
npm install git+ssh://git@github.com:MpesaFlow/mpesaflow-node.git
```
...
const client = new Mpesaflow({
  apiKey: process.env['MPESAFLOW_API_KEY'], // This is the default and can be omitted
});
...
async function main() {
  const params: Mpesaflow.PaymentCreateParams = {
    accountReference: 'accountReference',
    amount: 0,
    phoneNumber: 'phoneNumber',
    transactionDesc: 'transactionDesc',
  };
  const payment: Mpesaflow.PaymentCreateResponse = await client.payments.create(params);
}
...
await client.payments.create({ accountReference: 'accountReference', amount: 0, phoneNumber: 'phoneNumber', transactionDesc: 'transactionDesc' }, {
  maxRetries: 5,
});
```
...
add the following import before your first import `from "Mpesaflow"`:
