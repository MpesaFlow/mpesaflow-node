...
import { APIUserAbortError } from 'mpesaflow';
import { Headers } from 'mpesaflow/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Mpesaflow({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['MPESAFLOW_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Mpesaflow({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MPESAFLOW_BASE_URL'] = 'https://example.com/from_env';
      const client = new Mpesaflow({ apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new Mpesaflow({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://dev.mpesaflow.com/v1');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new Mpesaflow({ apiKey: 'My API Key', environment: 'production' }),
      ...
      const client = new Mpesaflow({ apiKey: 'My API Key', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://dev.mpesaflow.com/v1');
    });
  });

  test('maxRetries option is correctly set', () => {
    const client = new Mpesaflow({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Mpesaflow({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['MPESAFLOW_API_KEY'] = 'My API Key';
    const client = new Mpesaflow();
  ...
  test('with overriden environment variable arguments', () => {
    // set options via env var
    process.env['MPESAFLOW_API_KEY'] = 'another My API Key';
    const client = new Mpesaflow({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Mpesaflow({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Mpesaflow({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Mpesaflow({ apiKey: 'My API Key', fetch: testFetch });
